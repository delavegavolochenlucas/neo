...
AREAS = ["programação", "IA", "negócios", "psicologia", "comportamento", "frontend", "backend"]

_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_FILE = os.path.join(_BASE_DIR, "dados.db")
...
# Insere um registro de sessão encerrada com a data atual.
def registrar_sessao(estudado):
    con = sqlite3.connect(DB_FILE)
    cur = con.cursor()
    cur.execute(
        "INSERT INTO sessoes (data, estudado) VALUES (?, ?)",
        (datetime.now().isoformat(), estudado)
    )
    con.commit()
    con.close()


# Consulta a tabela sessoes e retorna estatísticas de engajamento.
# COUNT(*) conta total, WHERE filtra os últimos 7 dias, ORDER BY DESC pega a mais recente.
def analisar_dados():
    con = sqlite3.connect(DB_FILE)
    cur = con.cursor()
    ...
    """)
    ultimos_7_dias = cur.fetchone()[0]

    cur.execute("SELECT data FROM sessoes ORDER BY data DESC LIMIT 1")
    row = cur.fetchone()
    mais_recente = datetime.fromisoformat(row[0]).strftime("%d/%m/%Y") if row else None

    cur.execute("SELECT data, estudado FROM sessoes ORDER BY data DESC LIMIT 30")
    ultimas_30 = cur.fetchall()
    con.close()

    hoje = datetime.now()
    dias_por_area = {}
    for area in AREAS:
        dias_por_area[area] = None
        for data_str, estudado in ultimas_30:
            if area.lower() in estudado.lower():
                dias_por_area[area] = (hoje - datetime.fromisoformat(data_str)).days
                break
...
def construir_system(estado):
    ...
    # Injeta estatísticas de engajamento para o agente ter noção do ritmo de uso
    dados = analisar_dados()
    contexto_dados = (
        f"\n\nEstatísticas de uso: {dados['total']} sessões no total, "
        f"{dados['ultimos_7_dias']} nos últimos 7 dias"
        + (f", última em {dados['mais_recente']}." if dados["mais_recente"] else ".")
    )

    # Injeta cobertura por área para o agente identificar o que está sendo negligenciado
    linhas_areas = []
    for area, dias in dados["dias_por_area"].items():
        if dias is None:
            linhas_areas.append(f"- {area}: nenhuma menção nas últimas 30 sessões")
        else:
            linhas_areas.append(f"- {area}: há {dias} dia(s)")
    contexto_areas = "\n\nCoverage de áreas de estudo (últimas 30 sessões):\n" + "\n".join(linhas_areas)

    return base + contexto_tempo + contexto_ausencia + contexto_sessoes + contexto_dados + contexto_areas
